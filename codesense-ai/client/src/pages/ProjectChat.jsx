    ...
    <div
      className={cn(
        ...
        isUser
          ...
          : "bg-accent/10 border-accent/30 text-accent shadow-[0_0_15px_rgba(99,102,241,0.2)]"
      )}
    >
      ...
      <div className="max-w-4xl mx-auto w-full px-4 flex flex-col h-full relative z-10">
        <header className="py-4 flex items-center justify-between border-b border-white/5 bg-ink/80 backdrop-blur-md sticky top-0 z-20">
          ...
          <button
            ...
            disabled={readmeBusy}
            className="shrink-0 text-xs font-medium px-4 py-2 rounded-lg bg-panel2 border border-line text-paper hover:bg-panel hover:border-accent hover:text-accent transition-all disabled:opacity-50 flex items-center gap-2 group"
          >
            {readmeBusy ? (
              <><Loader2 className="w-3.5 h-3.5 animate-spin text-accent" /> Generating...</>
            ) : (
              <><Sparkles className="w-3.5 h-3.5 text-amber group-hover:text-accent transition-colors" /> Auto-Readme</>
            )}
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto py-6 space-y-6 scrollbar-hide">
          <AnimatePresence>
            {readme && (
              <motion.div 
                ...
                className="bg-panel/80 backdrop-blur border border-accent/30 rounded-2xl p-5 mb-6 shadow-2xl relative group"
              >
          ...
          {messages.length === 0 && !readme && (
            ...
            >
              <div className="w-16 h-16 rounded-2xl bg-accent/10 border border-accent/20 flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(99,102,241,0.15)]">
              ...
              <div className="flex flex-wrap justify-center gap-3 max-w-2xl">
                {SUGGESTIONS.map((s, i) => (
                  <motion.button
                    ...
                    className="text-sm font-medium text-ghost bg-panel border border-line rounded-xl px-4 py-2.5 hover:border-accent/50 hover:text-accent hover:shadow-[0_0_15px_rgba(99,102,241,0.15)] transition-all hover:-translate-y-0.5"
                  >
                    {s}
                  </motion.button>
                ))}
              ...
              <div
                className={cn(
                  ...
                  m.role === "user" 
                    ...
                    : "bg-panel/50 backdrop-blur-sm border border-accent/20 text-paper/90"
                )}
              >
                ...
                {m.citedFiles && m.citedFiles.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-line/50">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {m.citedFiles.map((f) => (
                        <span key={f} className="text-xs font-mono text-accent bg-accent/10 border border-accent/20 rounded-md px-2 py-1 flex items-center gap-1.5">
          ...
          <AnimatePresence>
            {asking && (
              ...
              >
                ...
                <div className="rounded-2xl p-4 bg-panel/50 backdrop-blur-sm border border-accent/20 flex items-center">
                  <TypingDots />
                </div>
              </motion.div>
            )}
        ...
        <div className="py-4 bg-ink">
          <form 
            onSubmit={handleAsk} 
            className="relative flex items-center bg-panel border border-line rounded-2xl p-2 shadow-[0_0_30px_rgba(0,0,0,0.5)] focus-within:border-accent/50 focus-within:ring-1 focus-within:ring-accent/50 transition-all"
          >
            <input
              ref={inputRef}
              value={question}
            ...
            <button
              ...
              className="p-2.5 rounded-xl bg-accent text-white font-semibold hover:bg-accent/90 transition-all disabled:opacity-50 disabled:bg-panel2 disabled:text-ghost"
            >
